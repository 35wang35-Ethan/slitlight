...
document.querySelectorAll('[data-link]').forEach(el=>{const value=content[el.dataset.link]?.trim();if(!value){el.hidden=true;return}el.href=el.dataset.link==='email'?`mailto:${value}`:value});
document.querySelectorAll('[data-phone]').forEach(el=>{const value=content.phone?.trim();if(!value){el.hidden=true;return}el.href=`tel:${value.replace(/[^+\d]/g,'')}`});
